<script lang="ts">
    ...
    import { timeStartFilter,timeStartField} from './store'
    import {showSubmit} from './store'
    ...
    $: if (time_fmt) {
        ...
        if (!ret.isValid) {
            ...
            time = ret
            $timeStartField.hour = time.hour
            $timeStartField.minute = time.minute
            $timeStartFilter = time.hour + time.minute / 60;
        }
        if (time.minute % 10 != 0) {
